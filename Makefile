...
.PHONY: all test difftest schemediff compile clean help

all: test

help:
	@echo "make test       — run ERT suite (hermetic, no external deps)"
	@echo "make difftest   — run SBCL-oracle CL differential suite (needs sbcl)"
	@echo "make schemediff — run Guile-oracle Scheme differential suite (needs guile)"
